...
plot(t,y1,type="l",col="red",xlab="x values", ylab= "y values")
lines(t,y2,col="green")
lines(x,square(x),col='blue',type='l')
...
The column names are: Sepal length, sepal width, petal length, petal width, and species.
...
```{r}
  ...
  library(dplyr)
  setosa = head(dplyr::filter(iris, Species == 'setosa'))
  versicolor = head(dplyr::filter(iris, Species == 'versicolor'))
  virginica = head(dplyr::filter(iris, Species == 'virginica'))
...
```{r}
boxplot(setosa$Petal.Length, versicolor$Petal.Length, virginica$Petal.Length, names=c("setosa", "versicolor", "virginica"), xlab = "Species", ylab = "Petal Length")
...
```{r}
boxplot(setosa$Sepal.Width, versicolor$Sepal.Width, virginica$Sepal.Width, names=c("setosa", "versicolor", "virginica"), xlab = "Species", ylab = "Sepal Width")
```

Note at least two observations that you can make looking at these boxplots. 

There is more variance in speal width than in petal length. Also there seems to be a correlation between species having a small speal width but a bigger petal length and vice versa. 
...
```{r}
  hist(samp_data)
  hist(samp_data,breaks = 1.0, main = "With breaks 1.0")
  hist(samp_data,breaks = 1.5, main = "With breaks 1.5")
  hist(samp_data,breaks = 2.0, main = "With breaks 2.0")
  hist(samp_data,breaks = 2.5, main = "With breaks 2.5")
  hist(samp_data,breaks = 3.0, main = "With breaks 3.0")
  hist(samp_data,breaks = 3.5, main = "With breaks 3.5")
  hist(samp_data,breaks = 4.0, main = "With breaks 4.0")
  hist(samp_data, breaks = 4.5, main ="With breaks 4.5")
  
```

What are some good values for the "breaks" parameter? Why?

As we see from the histograms, the higher the breaks parameter is,
the more populated our cells are. This means that we are able to analyze
our data more clearly.


Is the sample mean a good measure of location for this dataset? Why?

The mean of the sample data is 2.53215, according to the mean() function.
As we look at our histograms, we see that every break always includes the mean
in the populated data cells as part of our data set, meaning that it is 
a good measure of location.
